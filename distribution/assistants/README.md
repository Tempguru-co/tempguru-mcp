...
| Pi + Prime Agent (`tempguru-pi` npm package) | Agent Skill + native tools | 🟡 repository `1.7.0` candidate with 8 runtime-adapted skills, 9 native tools, a non-PII buyer-form handoff, and automatic `pi` / `prime-agent` attribution; official Prime v0.7.0 runtime smoke passed, while npm publication remains pending |
