...
// plus public/llms.txt and public/llms-full.txt so the bundle is discoverable
// both as a concise index and as a deterministic single-file export.
//
...
const rateIndexMeta = load("rate-index-meta.json");
...
const readSkill = (f) => {
  ...
  const descriptionValue = descriptionIndex >= 0
    ? frontmatterLines[descriptionIndex].slice("description:".length).trim()
    : "";
  const foldedLines = descriptionIndex >= 0
    ? frontmatterLines.slice(descriptionIndex + 1)
    : [];
  const foldedEnd = foldedLines.findIndex((line) => line && !/^\s+/.test(line));
  const desc = descriptionValue.startsWith(">") || descriptionValue.startsWith("|")
    ? foldedLines
        .slice(0, foldedEnd === -1 ? undefined : foldedEnd)
        .map((line) => line.trim())
        .filter(Boolean)
        .join(" ")
  ...
  return { desc, body, title };
};

const SKILL_WORKFLOW_META = {
  "event-staffing-ordering": {
    label: "Event staffing ordering",
    blurb: "the conversational planning and booking flow",
    primary_tool: "plan_staffing",
    tags: ["workflow", "ordering", "event-staffing"],
  },
  "event-staffing-compliance": {
    label: "Event staffing compliance",
    blurb: "worker-classification and state-rules guidance",
    primary_tool: "get_compliance_by_state",
    tags: ["workflow", "compliance", "event-staffing"],
  },
  "staffing-plan-from-event-brief": {
    label: "Staffing plan from an event brief",
    blurb: "extract and price staffing requirements from an RFP, BEO, or run of show",
    primary_tool: "plan_staffing",
    tags: ["workflow", "event-brief", "rfp", "beo", "planning"],
  },
  "urgent-event-backfill": {
    label: "Urgent event backfill",
    blurb: "handle no-shows, vendor cancellations, and same-week staffing requests",
    primary_tool: "plan_staffing",
    tags: ["workflow", "urgent", "backfill", "event-staffing"],
  },
  "staffing-agency-partner-growth": {
    label: "Staffing agency partner growth",
    blurb: "route W-2 staffing agencies into TempGuru's supply-side partner channel",
...
const A2A_ENDPOINT = publicFacts.agentInterfaces.a2a.url;
...
const KNOWLEDGE_ORIGIN = new URL(BUNDLE_URL).origin;
const LLMS_SHORT_URL = `${KNOWLEDGE_ORIGIN}/llms.txt`;
const LLMS_FULL_URL = `${KNOWLEDGE_ORIGIN}/llms-full.txt`;
const AGENT_DOCS_URL = `${SITE}/ai-agents`;
const AUTH_GUIDE_URL = `${SITE}/auth.md`;
const PUBLIC_FACTS_URL = `${SITE}/.well-known/tempguru-facts.json`;
const AGENT_CARD_URL = `${SITE}/.well-known/agent-card.json`;
const BUILD_NOTE =
...
const policyBody = (policy) => {
  ...
  if (policy.confirm_with_coordinator) {
    lines.push("**Coordinator confirmation required.** Confirm event-specific terms with a TempGuru coordinator before relying on a timing, fee, or exception that is not stated above.");
  }
...
const cityListSection = (map) =>
  [...map.keys()].sort().map((state) => {
    const list = map.get(state)
      ...
      .map((c) => `[${c.name}](${c.detail_url ?? `${SITE}/insights/${c.slug}`})`)
      .join(", ");
    return `- **${state}** (${map.get(state).length}): ${list}`;
  }).join("\n");

const tierCounts = cities.cities.reduce(
...
- Phone: (904) 206-8953
- AI / developer documentation: ${AGENT_DOCS_URL}
...
// ROOT: policies.md
// ══════════════════════════════════════════════════════════════════════════
write("policies.md", doc(
  {
    type: "Policy Catalog",
    title: "Booking and Procurement Policies",
    description: "Published TempGuru event-staffing policies, with unresolved terms explicitly deferred to a coordinator rather than guessed.",
    source: "content/mcp-data/policies.json",
    tags: ["policies", "booking", "procurement", "event-staffing"],
    timestamp: `${policies._meta.updated}T00:00:00Z`,
  },
  `# Booking and Procurement Policies

${policies._meta.scope}

> ${policies._meta.disclaimer}

Confirmed claims below may be used as written. Any section marked **Coordinator
confirmation required** contains an unpublished detail; do not infer a number,
deadline, fee, or exception.
...
write("rate-index.md", doc(
  {
    ...
    edition: "2026",
    data_version: rateIndexMeta.version,
  ...
  write(`cities/${tier}-markets.md`, doc(
    {
      type: "Market List",
      title: `${label} Markets`,
      description: `TempGuru's ${tierCounts[tier]} configured ${tier}-tier event-staffing market entries, grouped by state/province.`,
      tier,
      count: tierCounts[tier],
      tags: ["market-catalog", "cities", tier],
      timestamp: TS,
    },
...
// workflows/
// ══════════════════════════════════════════════════════════════════════════
const canonicalSkillWorkflowLinks = SKILLS.map((slug) => {
  const meta = SKILL_WORKFLOW_META[slug];
  return `- [${meta.label}](${slug}.md), ${meta.blurb} (canonical skill)`;
}).join("\n");
...
for (const slug of SKILLS) {
  const skill = readSkill(`${slug}.md`);
  const meta = SKILL_WORKFLOW_META[slug];
  write(`workflows/${slug}.md`, doc(
    {
      type: "Workflow",
      title: skill.title ?? meta.label,
      description: skill.desc ?? meta.blurb,
      source: `content/skills/${slug}.md`,
      primary_tool: meta.primary_tool,
      tags: meta.tags,
      timestamp: TS,
    },
    skill.body,
...
write("reference/source-map.md", doc(
  {
    type: "Source Map",
    title: "Source Map",
    ...
    tags: ["reference", "provenance", "drift-proof"],
    timestamp: TS,
  },
...
The same data powers the live tools (MCP at \`${MCP_ENDPOINT}\`, REST at
\`${API_BASE}\`), the MCP-hosted \`llms.txt\` exports, and the \`tempguru.co\`
...
- [A2A JSON-RPC 1.0 endpoint](${A2A_ENDPOINT}): deterministic structured staffing-plan and public catalog skills
- [A2A Agent Card](${AGENT_CARD_URL})
- [Agent authentication guide](${AUTH_GUIDE_URL}): public MCP, A2A, and lookup operations require no account or OAuth
- [Evidence-gated public facts](${PUBLIC_FACTS_URL})
- Quote-submission boundary: MCP request_quote only returns a buyer-operated form and never accepts contact details; REST submitQuoteRequest is a contact-bearing write and requires explicit confirmation
...
- Install the skills and attributed MCP action layer: https://github.com/Tempguru-co/tempguru-mcp#connect

## Contact
- [Get staffing](${SITE}/get-staffing)
- [AI / developer docs](${AGENT_DOCS_URL})
- Email: megan@tempguru.co · Phone: (904) 206-8953
`;
writeFileSync(join(publicDir, "llms.txt"), llmsTxt);

const llmsFullPaths = writtenPaths
...
const llmsFullTxt =
  `${llmsTxt}\n---\n\n# Full Open Knowledge Format export\n\n` +
  `Compact index: ${LLMS_SHORT_URL}\n\n` +
  `This section contains all ${llmsFullPaths.length} Markdown documents from the deterministic TempGuru OKF bundle.\n\n` +
  llmsFullPaths
    .map((path) =>
      `## Included document: /okf/${path}\n\n` +
      `Canonical URL: ${BUNDLE_URL}/${path}\n\n` +
      readFileSync(join(outDir, path), "utf8").trim(),
    )
    .join("\n\n---\n\n") +
  "\n";
...
writeFileSync(
  ...
  readFileSync(join(repoRoot, "content", "auth.md"), "utf8"),
);
...
// A gzip stream made from uncompressed DEFLATE blocks is larger than zlib's
// level-9 output, but its bytes do not depend on the host's zlib version.
// That makes the published checksum stable across macOS, Linux CI, and Vercel.
const crc32 = (bytes) => {
  let crc = 0xffffffff;
...
const deterministicGzip = (bytes) => {
  // RFC 1952: no optional fields, fixed mtime, neutral OS identifier.
  ...
  if (bytes.length === 0) {
    ...
    for (let offset = 0; offset < bytes.length; offset += 0xffff) {
      ...
      block[0] = final ? 0x01 : 0x00; // BFINAL + stored-block BTYPE + padding.
      block.writeUInt16LE(length, 1);
      block.writeUInt16LE((~length) & 0xffff, 3);
      parts.push(block, bytes.subarray(offset, offset + length));
    }
  }
...
let tarballMetadata = {};
try {
  // Build USTAR bytes directly instead of shelling out to platform-specific
  // tar implementations. Fixed ownership, modes, timestamps, ordering, and
  // the host-independent gzip writer make the archive (and advertised SHA-256)
  // identical on macOS, Linux CI, and Vercel.
  const writeTarText = (header, value, offset, length) => {
    const bytes = Buffer.from(value, "utf8");
    if (bytes.length > length) throw new Error(`USTAR field too long: ${value}`);
    bytes.copy(header, offset);
  };
  const writeTarOctal = (header, value, offset, length) => {
    const encoded = Math.trunc(value).toString(8).padStart(length - 1, "0") + "\0";
    writeTarText(header, encoded, offset, length);
  };
  const splitTarPath = (path) => {
    ...
    throw new Error(`USTAR path too long: ${path}`);
  };
  const tarEntry = (path, body) => {
    ...
    writeTarOctal(header, Math.floor(new Date(TS).getTime() / 1000), 136, 12);
  ...
  const tarParts = [];
  for (const relPath of [...writtenPaths].sort()) {
    tarParts.push(...tarEntry(`okf/${relPath}`, readFileSync(join(outDir, relPath))));
  }
  tarParts.push(Buffer.alloc(1024));
  const tarBytes = Buffer.concat(tarParts);
  const tarGzPath = join(publicDir, "okf.tar.gz");
  const tarballBytes = deterministicGzip(tarBytes);
  writeFileSync(tarGzPath, tarballBytes);
  tarballMetadata = {
  ...
  throw new Error("Failed to build the deterministic OKF archive", { cause: error });
}
...
const okfDiscovery = {
  ...
  bundle: {
    root: `${BUNDLE_URL}/`,
    index: `${BUNDLE_URL}/index.md`,
    tarball: TARBALL_URL,
    file_count: writtenPaths.length,
    ...tarballMetadata,
  },
};
...
const robots =
  ...
  `# See /llms.txt, /llms-full.txt, and /.well-known/okf.json for the bundle and discovery doc.\n` +
  `User-agent: *\n` +
  `Allow: /\n\n` +
...
console.log(`Wrote public/llms.txt, public/llms-full.txt, public/auth.md, public/.well-known/okf.json, public/sitemap.xml, public/robots.txt, ${tarballNote}`);
