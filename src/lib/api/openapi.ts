...
export function buildOpenApiSpec(now = new Date()) {
  return {
    ...
    info: {
      ...
      version: pkg.version,
      summary:
        `${APPROVED_SCALE_DESCRIPTION} Eight read-only operations plus one opt-in quote-request submission.`,
      description: [
        ...
        "",
        `**Evidence-verified public scale.** ${APPROVED_SCALE_DESCRIPTION} Stable claim IDs: ${APPROVED_SCALE_CLAIM_IDS.map((claimId) => `\`${claimId}\``).join(", ")}.`,
        "",
