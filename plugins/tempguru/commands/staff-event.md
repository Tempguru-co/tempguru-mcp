...
1. Call `plan_staffing` first with everything provided (city, date, event type, roles + headcount, attendees). It returns coverage, per-role W-2 rate math, lead-time guidance, and state compliance flags in one call. Retain the returned `plan_id` when the plan is complete.
...
4. Only after I explicitly confirm, collect my contact name, email, company, and event name. Submit the confirmed roles and shifts once with `request_quote`, passing the retained `plan_id`, `source_platform: "claude-code"`, `skill_id: "event-staffing-ordering"`, and `skill_version: "1.5.0"`. Retain the returned TG reference. A TempGuru coordinator replies with a binding quote within one business day; nothing is reserved and no payment is due until I approve the quote.
