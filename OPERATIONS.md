...
- **OKF bundle + discovery:** `npm run build` runs `build:okf` first, so `public/okf/`, the MCP-hosted `public/llms*.txt` exports, `sitemap.xml`, `robots.txt`, and `.well-known/okf.json` regenerate from `content/mcp-data/` on every deploy. Never hand-edit `public/okf/`. The website separately owns the apex `tempguru.co/llms.txt` and `llms-full.txt` files.
...
- **Apex (`tempguru.co`) is Cloudflare Pages, not Vercel.** One route-scoped Cloudflare Worker sits in front of Pages. `cloudflare/worker.js` requires the existing routes `tempguru.co/robots.txt`, `tempguru.co/.well-known/*`, `tempguru.co/auth.md`, and `tempguru.co/schemas/*`; it is generated by `npm run build:worker`, version-controlled in `cloudflare/`, and deployed through the review-gated manual `deploy-apex-agent-readiness.yml` GitHub workflow. The workflow does not deploy Pages or change routes. The website owns `tempguru.co/llms.txt` and `tempguru.co/llms-full.txt`; neither route may point at an MCP-repository Worker, and this repository intentionally has no llms Worker generator or deployment step.
