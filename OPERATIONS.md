...
- **Apex (`tempguru.co`) is Cloudflare Pages, not Vercel.** Two route-scoped Cloudflare Workers sit in front of Pages. `cloudflare/worker.js` requires the existing routes `tempguru.co/robots.txt`, `tempguru.co/.well-known/*`, `tempguru.co/auth.md`, and `tempguru.co/schemas/*`; `cloudflare/llms-worker.js` requires `tempguru.co/llms.txt` and `tempguru.co/llms-full.txt`. Both are generated (`npm run build:worker`, `npm run build:llms-worker`), version-controlled in `cloudflare/`, and deployed through the review-gated manual `deploy-apex-agent-readiness.yml` GitHub workflow. The workflow uses the existing Worker names, does not alter routes or deploy Pages, and runs the live canary after upload. Dashboard copy/paste deployment is retired.
