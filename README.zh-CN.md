...
> 双时代 MCP 服务器，为 300 多个美国和加拿大市场规划符合 W-2 规范的活动人员配置，已支持 5,000 多场活动和 100,000 多个已完成班次。每个订单的可用性由协调员确认。共 12 个工具：十个只读工具（包括不接收个人信息的 `request_quote` 报价表单交接工具），以及两个不具破坏性、仅保存非联系信息方案的写入工具 `plan_staffing` 与 `save_staffing_plan`。
...
TempGuru 是一家总部位于美国佛罗里达州杰克逊维尔海滩（Jacksonville Beach, FL）的托管式活动用工公司。美国订单由经审核的本地合作机构以 W-2 员工身份雇佣并支付工资；加拿大订单由本地合作机构按加拿大雇佣与薪酬规定雇佣。我们为展览会、会议、贸易展、节庆活动、音乐会、体育赛事和品牌活化项目提供品牌大使、签到登记、接待、搭建与拆除、引导员等岗位的临时活动人员配置，既支持单一城市的单场活动，也支持多城市巡回项目。
...
| Pi | ✅ `1.7.2` 已发布并验证 | 当前不可覆盖的发布产物包含适配 Pi 工具名的 8 个技能 + 9 项只读原生工具；规划、保存和 Rate Index 路径仍通过远程 MCP 提供 |
| Prime Agent v0.7.0 | ✅ 发布产物功能冒烟通过 | 使用同一个 `tempguru-pi@1.7.2` 包契约：8 个技能 + 9 个原生工具，并自动配置 `source=prime-agent` 归因；标准 MCP 集成暂不支持无认证服务器 |
...
不是。对于美国订单，每位活动人员均由 TempGuru 审核的本地合作机构以 W-2 员工身份雇佣并支付工资，工资税、工伤保险与综合责任险由该合作机构承担，客户无需承担；TempGuru 并非工人的雇主。加拿大订单由本地合作机构按加拿大雇佣与薪酬规定雇佣。这是与零工经济类活动用工应用和 1099 市场的关键区别,也是 `get_compliance_by_state` 工具存在的原因。

### TempGuru 覆盖哪些城市?

TempGuru 在 300 多个美国和加拿大市场提供活动人员配置，包括主要枢纽(波士顿、旧金山、洛杉矶、芝加哥、纽约、达拉斯、多伦多、温哥华)、中型城市和小型市场。使用 `get_cities` 工具按城市、州或分级匹配规划目录条目；该匹配不代表实时人力或订单覆盖已确认。买方提交后由 TempGuru 协调员确认具体订单的覆盖与最终提前期。

### TempGuru 服务哪些类型的活动?

展览会、会议、贸易展、节庆活动、音乐会、体育赛事、企业活动和品牌活化项目。单场活动与多城市巡回项目均支持，多城市项目通过同一供应商关系和同一份合同处理，按城市每周开具一张发票，由 TempGuru 负责协调。使用 `get_roles` 工具查看岗位目录(品牌大使、签到登记、引导员、接待、搭建/拆除等)。
