...
5. On the user's explicit confirmation, collect contact name/email/company and call `request_quote` with the existing `plan_id` when available. A coordinator replies with a binding quote within one business day; no payment until the user approves.
